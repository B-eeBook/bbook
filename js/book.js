...
<<<<<<< HEAD
  // Chỉ hiển thị pagination trên trang danh sách sách
  const isListPage = window.location.pathname.includes('index.html') ||
...
<<<<<<< HEAD
  ...
  if (breadcrumb) {
    breadcrumb.innerHTML = `
      ...
      <a href="category.html?cat=${encodeURIComponent(book.category)}">${book.category}</a> › 
...
export function renderCategorySidebar() {
  ...
  const dropdownContainer = document.querySelector("#category-dropdown ul");
  if ((!sidebarContainer && !dropdownContainer) || books.length === 0) return;
=======
        <p class="book-price">${book.price.toLocaleString()}đ</p>
      </a>`;
    container.appendChild(div);
...
<<<<<<< HEAD
  const categoryHTML = categories.map(cat => {
    ...
    return `<li><a href="category.html?cat=${encodeURIComponent(cat)}" class="${isActive}">${cat}</a></li>`;
  }).join("");
